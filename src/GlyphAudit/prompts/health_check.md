...
The tool ran a tiered comparison between a target font and one or more reference fonts:

- **Tier 1 — codepoint match**: every encoded glyph in the target font is paired with the reference's glyph for the same Unicode codepoint, and their advance widths are compared.
...
- **Tier 3 — internal-only**: target glyphs that have neither a codepoint nor a recognised feature suffix (components, ligature parts, locl variants). Pay special attention to any flagged as "variant of unencoded base" — those mean a feature variant exists but its base glyph is missing, which is almost always a bug.

Tolerance for "match" was ±{tolerance} unit(s) (per 1000 UPM unless `--no-normalize-upm` was used).
Filter applied to target glyphs: {filter_label}.
Number of target/reference pairs reported: {pair_count}.

## What to look for

Prioritise findings in this order:

1. **Duplicate or wrong unicode assignments** — e.g. an uppercase glyph holding both an uppercase and a lowercase codepoint, blocking creation of a separate lowercase. Symptom: the target glyph name looks wrong for the codepoint shown (e.g. `U-cy` mapped to U+0443 lowercase у), or unusually large advance deltas concentrated on what should be a single character.
...
5. **Missing-in-reference glyphs** — note them only briefly; they're often expected because the target font extends beyond the reference's character set.
